...
- ⏱️ 智能计时：下载成功才重置计时器；失败时采用指数退避重试，避免频繁请求服务器
- 🌐 网络容错：下载自带 3 次指数退避重试，网络抖动不影响使用
...
- **在线图片偏好**：选择图源（`all` / `konachan` / `yandere`）和尺寸（`pc` / `mobile`）。
- **星标收藏夹管理**：
...
使用仓库内附带的 PyInstaller 配置打包（无需控制台窗口）：
...
生成的 `dist/wallpaper_changer.exe` 可独立运行（`--silent` 参数同样生效）。
